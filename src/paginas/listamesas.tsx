...
const ListaMesas: React.FC = () => {
  ...
  const [formData, setFormData] = useState<{ Numero_mesa: number | ''; ID_Salon: number | '' }>({
    Numero_mesa: '',
    ID_Salon: ''
  });
  ...
  if (cargando) return <div className="lista-mesas-container">Cargando mesas...</div>;
      ...
      {showForm && (
        <form onSubmit={handleFormSubmit} style={{ margin: '1em 0' }}>
          <input
            ...
            onChange={e => setFormData({ ...formData, Numero_mesa: parseFloat(e.target.value) || '' })}
            required
          />
          <select
            value={formData.ID_Salon}
            onChange={e => setFormData({ ...formData, ID_Salon: Number(e.target.value) })}
            required
          >
          ...
          {mesas.map((mesa) => (
            <tr key={mesa.ID_Mesa}>
              ...
              <td>
                ...
                <button onClick={() => handleDelete(mesa.ID_Mesa)}>Eliminar</button>
              </td>
            </tr>
          ))}
        </tbody>
